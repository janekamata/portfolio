...
class MazeCell {
  ...
  //produces the to neighbor of this cell in the given direction
  ...
  // produces the from neighbor of this cell in the given direction
...
class MazeWorld extends World {
  ...
  // This field is not final because it is used to determine
  // the gameType based on user input and restrict how user
  // input works after they have selected a gameType
  ...
  public void onKeyEvent(String key) {
    ...
    if ((key.equals("up")) && !this.gameType.equals("b") && !this.gameType.equals("d")) {
      this.gameType = "manual";
      this.graph.movePlayer("up");
    }
    // traverse the maze down
    else if ((key.equals("down")) && !this.gameType.equals("b") && !this.gameType.equals("d")) {
      this.gameType = "manual";
      this.graph.movePlayer("down");
    }
    // traverse the maze left
    else if ((key.equals("left")) && !this.gameType.equals("b") && !this.gameType.equals("d")) {
      this.gameType = "manual";
      this.graph.movePlayer("left");
    }
    // traverse the maze right
    else if ((key.equals("right")) && !this.gameType.equals("b") && !this.gameType.equals("d")) {
      this.gameType = "manual";
      this.graph.movePlayer("right");
    }
    // breadth-first search
    else if ((key.equals("b") && !this.gameType.equals("manual") && !this.gameType.equals("d"))) {
      this.gameType = "b";
      this.graph.newPath("b");
    }
    // depth-first search
    else if ((key.equals("d")) && !this.gameType.equals("manual") && !this.gameType.equals("b")) {
      this.gameType = "d";
      this.graph.newPath("d");

    }
    // create a new random maze
    else if (key.equals("n") && this.gameType.equals("start")) {
  ...
  public WorldScene makeScene() {
    ...
    result.setPixels(0, 0, size, size, Color.magenta);
    canvas.placeImageXY(this.graph.drawPath(result, size), width / 2, height / 2);
    result.setPixels(width - size, height - size, size, size, Color.magenta);
    canvas.placeImageXY(this.graph.drawPlayer(result, size), width / 2,
        height / 2);
...
class MazeGraph {
  ...
  private final ArrayList<Edge> pathSoFar;
  ...
  MazeGraph(int height, int width) {
    ...
    this.pathSoFar = new ArrayList<Edge>();
  }

  // updates pathSoFar with the next edge in the path
  void nextCellInPath() {
    if ((this.pathSolverEdges.size() != 0)
        && (this.pathSoFar.size() < this.pathSolverEdges.size())) {
      this.pathSoFar.add(this.pathSolverEdges.get(this.pathSoFar.size()));
    }
  }
  ...
  boolean shouldEnd() {
    ...
    else if (this.pathSolverEdges.size() > 0) {
      return this.pathSoFar.size() == this.pathSolverEdges.size();
    }
    else {
      return false;
    }
  ...
  void movePlayer(String direction) {
    ...
    if (this.playerCells.size() == 0) {
    ...
    else if ((this.playerCells.get(this.playerCells.size() - 1)
        .findNeighborFrom(direction) != null)
      ...
      this.playerCells.add(
        ...
        && this.playerCells.get(this.playerCells.size() - 1).findNeighborTo(direction)
            .hasEdge(this.mazeEdges)) {
      this.playerCells
          .add(this.playerCells.get(this.playerCells.size() - 1).findNeighborTo(direction)
              .getTo());
    }
  }
  ...
  WorldImage drawPath(ComputedPixelImage result, int size) {
    ...
    for (int e = 0; e < this.mazeCells.size(); e += 1) {
      this.mazeCells.get(e).drawCell(result, size, this.pathSoFar);
    }
  ...
  ArrayList<Edge> kruskalsAlgorithm(ArrayList<MazeCell> cells) {
    ...
    for (MazeCell cell : cells) {
      for (Edge e : cell.getEdges()) {
        Edge tempEdge = new Edge(e.getTo(), e.getFrom());
        if (!(tempEdge.hasEdge(worklist))) {
          worklist.add(e);
        }
      }
    }
