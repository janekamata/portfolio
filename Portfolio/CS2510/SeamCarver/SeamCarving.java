...
// Design decision: class is named APixel instead of AIPixel for better readability
abstract class APixel implements IPixel {
  ...
  //observes the right of this pixel
...
class BorderPixel extends APixel {
  ...
  public void removePixelVertical() {
    // does nothing at a border pixel
  }
  ...
  // EFFECT: does not create energies in the column at this border pixel
  public void createEnergiesColumn() {
    // does nothing at a border pixel
  }

  // EFFECT: does not create energies in the row at this border pixel
  public void createEnergiesRow() {
    // does nothing at a border pixel
  }

  // EFFECT: does not check well-formed-ness of the column at this border pixel
  public void wellFormedColumn() {
    // does nothing at a border pixel
  }

  // EFFECT: does not check well-formed-ness of the row at this border pixel
  public void wellFormedRow() {
    // does nothing at a border pixel
  }

  // EFFECT: does not add this border pixel to a row
  public void addToListRow(ArrayList<APixel> firstRow) {
    // does nothing at a border pixel
  }

  // EFFECT: does not add this border pixel to a list
  public void removePixelHorizontal() {
    // does nothing at a border pixel
  }

  // adds the pixels to the neighbors of this border pixel
  ...
  // EFFECT: does not add this border pixel to the list
...
class ImagePixel extends APixel {
  ...
  // EFFECT: updates this image pixel's up, right, down, and left field
  public void updateNeighbors(APixel up, APixel right, APixel down, APixel left) {
    this.up = up;
    this.right = right;
    this.down = down;
  ...
  // EFFECT: updates this image pixel's inwardPixel field with the given pixel
  public void updateNeighbors(APixel inward) {
  ...
  public void wellFormedRow() {
    if (this.left.up != this.up.left) {
      ...
      throw new RuntimeException(
          "Not well formed: right-down and down-right of this image pixel");
    }
  ...
  // EFFECT: moves this image pixel and the ride pixels over by one to the left, closing in
  // the picture and reconnecting
  public void removePixelHorizontal() {
  ...
  // finds the neighbors of this image pixel and adds the 3 to an array list
  ...
  // EFFECT: adds the column of pixels to a list
  public void addToListColumm(ArrayList<APixel> firstColumn) {
...
class Graph {
  ...
  void createPixels() {
    ...
    // iterates over each row of pixels in the image and creates the pixels for each row
    for (int y = 0; y < this.originalHeight; y += 1) {
      ...
      if (y == 0) {
        // iterates over each column in the row of pixels in the image and creates border pixels
        // for each column
      ...
      // iterates over each column of pixels in the row of pixels and creates the pixels/borders
      for (int x = 0; x < this.originalWidth; x += 1) {
  ...
  void createGraph() {
    ...
    for (int y = 0; y < this.grid.size(); y += 1) {
      ...
      // bottom border
        ...
        // image pixels
        // iterates over the ArrayList at i in grid and creates the neighbors for the pixels in
        // the list
        for (int x = 0; x < currentRow.size(); x += 1) {
          // left border
          if (x == 0) {
  ...
  // EFFECT: determines if this graph is well formed and throws exceptions if it's not
  boolean wellFormed() {
  ...
  // EFFECT: sets the lowestVerticalSeam to the SeamInfo with the smallest
  // weight out of all the small seam infos
  ...
  boolean shouldEnd(int wDelta, int hDelta, int count) {
    if (count > 1) {
      return ((((int) this.image.getHeight() - 1) != (this.originalHeight - (Math.abs(hDelta))))
          || (((int) this.image.getWidth() - 1) != (this.originalWidth
              - (Math.abs(wDelta)))));
    }
    //return this.wellFormed();
    return (((int) this.image.getHeight() == 0) && ((int) this.image.getWidth() == 0));
  }

  //EFFECT:  sets the lowestHorizontalSeam to the SeamInfo with the
  // smallest weight out of all the small seam infos
  // from the list, using a comparator that compares their totalWeights that
  // should have been accumulated
  ...
  // EFFECT: big function!! takes the lowest horizontal seam of this graph and removes it
...
class SeamCarver extends World {
  ...
  //EFFECT: carves the image and changes the world according to specific key-presses
  // able to pause
  ...
  public void onKeyEvent(String key) {
    ...
    // removes vertical seam if onTick is paused
    ...
    // removes horizontal seam if onTick is paused
