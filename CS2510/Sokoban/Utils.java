...
class Utils {
  ...
  // produces the opposite direction from the given string direction
  ...
  // produces a cell based on the given name and the location (column, row)
  ICell makeCell(String given, int column, int row, boolean isGround) {
    ...
    else if (given.equals("I")) {
      return new Ice(coord);
    }
    else {
  ...
  ICell findCell(ArrayList<ICell> list, int x, int y) {
    for (int i = 0; i < list.size(); i += 1) {
      ICell cell = list.get(i);
      if (new Utils().samePosn(cell.accept(new CellPosnVisitor()), new Posn(x, y))) {
        return cell;
      }
    }
...
class ExamplesUtils {
  ...
  // tests and examples for opposite in Utils
  boolean testOpposite_Utils(Tester t) {
    return t.checkExpect(new Utils().opposite("up"), "down")
        && t.checkExpect(new Utils().opposite("down"), "up")
        && t.checkExpect(new Utils().opposite("left"), "right")
        && t.checkExpect(new Utils().opposite("right"), "left");
  }
  ...
  boolean testMakeCell_Utils(Tester t) {
    ...
    return t.checkExpect(new Utils().makeCell("_", 0, 0, false), blank)
        ...
        && t.checkException(
            new IllegalArgumentException("Invalid character given to build the level"),
            new Utils(),
  ...
  boolean testToLevelCells_Utils(Tester t) {
    ...
    return t.checkExpect(new Utils().toLevelCells(givenExLevelGround, true),
        givenExLevelGroundList)
  ...
  boolean testFindSize_Utils(Tester t) {
    ...
    String superShortExLevelContents = "W>_b__W";
    ...
    return t.checkExpect(new Utils().findSize(givenExLevelGround), new Posn(8, 7))
        ...
        && t.checkExpect(new Utils().findSize(superShortExLevelContents), new Posn(7, 1));
  }
  ...
  boolean testFindNext_Utils(Tester t) {
    ...
    return t.checkExpect(
        ...
        && t.checkExpect(
            new Utils().findNext(givenExLevelContentsList, "left", new Posn(4, 4),
                new Posn(8, 7)),
        ...
        && t.checkExpect(
            new Utils().findNext(givenExLevelContentsList, "down", new Posn(7, 6),
                new Posn(8, 7)),
