...
public abstract class AbstractPlayer implements Player {
  ...
  /**
   * Determines if the given coordinate has a collision with a ship in the given list of ships.
   *
  ...
  public List<Ship> setup(int height, int width, Map<ShipType, Integer> specifications) {
    ...
    while (attemptCount < AbstractPlayer.MAX_ATTEMPTS) {
      ships.clear();
      if (this.isPlacedShipsValid(ships, height, width, specifications)) {
  ...
  /**
   * Places ships based on the board dimensions and the specifications for the fleet size. Utilizes
   * the generate ship helper to place individual ships.
   *
   * @param ships          the ships list to be added to
   * @param height         the height of the board
   * @param width          the width of the board
   * @param specifications the specifications of the fleet of ships
   * @return the boolean representing if a valid placement was found
   */
  private boolean isPlacedShipsValid(Collection<Ship> ships, int height, int width,
  ...
  /**
   * Generates a ship based on the given ship type, dimensions of the board, and existing list of
   * ships.
   *
   * @param shipType      the ship type
   * @param height        the height of the board
   * @param width         the width of the board
   * @param existingShips the existing ships on the board
   * @return the ship generated based on the criteria; null if it is placed in an invalid place
   */
  ...
  public List<Coordinate> reportDamage(List<Coordinate> opponentShotsOnBoard) {
    if (opponentShotsOnBoard.isEmpty()) {
      throw new IllegalArgumentException("Opponent's shots cannot be empty.");
    }
